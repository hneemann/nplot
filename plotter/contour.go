...
// a contour nplot of the values in the GridXYZ field.
type Contour struct {
	GridXYZ GridXYZ

	// Levels describes the contour heights to nplot.
	Levels []float64
...
func (h *Contour) Plot(c draw.Canvas, plt *nplot.Plot) {
	if h.Min > h.Max {
...
func (h *Contour) naivePlot(c draw.Canvas, plt *nplot.Plot) {
	var pal []color.Color
	if h.Palette != nil {
		pal = h.Palette.Colors()
	}
...
func (h *Contour) GlyphBoxes(plt *nplot.Plot) []nplot.GlyphBox {
	c, r := h.GridXYZ.Dims()
