...
func TestLineWidth(t *testing.T) {
	...
	for _, w := range []vg.Length{-1, 0, 1} {
		for _, typ := range formats {
			p, err := lines(w)
			if err != nil {
				log.Fatalf("failed to create nplot for %v:%s: %v", w, typ, err)
			}

			c, err := p.WriterTo(width, height, typ)
			if err != nil {
				t.Fatalf("failed to render nplot for %v:%s: %v", w, typ, err)
			}

			var buf bytes.Buffer
			if _, err = c.WriteTo(&buf); err != nil {
				t.Fatalf("failed to write nplot for %v:%s: %v", w, typ, err)
			}
...
func lines(w vg.Length) (*nplot.Plot, error) {
	p, err := nplot.New()
	if err != nil {
		return nil, err
	}
